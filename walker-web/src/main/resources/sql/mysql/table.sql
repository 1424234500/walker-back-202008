---
---S_MTIME  VARCHAR2(32)	 long/字符串ymdhms 修改时间 自动设置	YYYY-MM-DD HH24:MM-SS:SSS
---S_FLAG   VARCHAR2(2)      0/1     假删除 配合S_MTIME增量查询
---


-- 用户 分表
-- CREATE DATABASE IF NOT EXISTS walker default charset utf8 COLLATE utf8_general_ci;
-- GRANT ALL PRIVILEGES ON *.* TO 'walker'@'%' IDENTIFIED BY 'qwer' WITH GRANT OPTION;


--测试表
--DROP TABLE STUDENT;
CREATE TABLE  IF NOT EXISTS  STUDENT (ID VARCHAR(40), NAME VARCHAR(200), S_MTIME VARCHAR(32), S_FLAG VARCHAR(4));


---
--tomcat web系统监控 统计
---
--文件映射表
DROP TABLE W_FILEINFO;
CREATE TABLE  IF NOT EXISTS  W_FILEINFO (ID VARCHAR(40), UPTIME VARCHAR(30), NAME VARCHAR(200), FILESIZE VARCHAR(10), TYPE VARCHAR(20), PATH VARCHAR(400), CHANGETIME VARCHAR(30), ABOUT VARCHAR(512), UPUSERID VARCHAR(40) );
TRUNCATE TABLE W_FILEINFO;
--[INFO][17:42:37] INSERT INTO FILEINFO(ID,                   UPTIME, NAME,FILESIZE,  TYPE,PATH,CHANGETIME               ,ABOUT,UPUSERID ) VALUES ('294818687017769_KLTCT8', SYSDATE,'2019-04-03 17-16-08 的屏幕截图.PNG'    ,'0'      ,  'PNG'   ,'/HOME/WALKER/TOMCAT/2019-04-03 17-16-08 的屏幕截图.PNG'    , TO_DATE('2019-05-24 17:42:37', 'YYYY-MM-DD HH24:MI:SS') ,'', ''   )  [0:0:0:0:0:0:0:1:44740][COM.WALKER.WEB.DAO.HIBERNATE.IMPL.BASEDAOIMPL.OUT(BASEDAOIMPL.JAVA:26)][HTTP-NIO-8080-EXEC-4]
--上传下载记录 + "(ID, FILEID, TYPE, COSTTIME, TIME),LANGUTIL.GETGENERATEID(), FILEID, TYPE, DETATIME, TIMEUTIL.GETTIMEYMDHMSS());
DROP TABLE W_FILE_DOWN_UP;
CREATE TABLE  IF NOT EXISTS  W_FILE_DOWN_UP (ID VARCHAR(40), FILEID VARCHAR(40),TYPE VARCHAR(40), COSTTIME VARCHAR(10), TIME VARCHAR(30) );
TRUNCATE TABLE W_FILE_DOWN_UP;

--接口访问统计控制
DROP TABLE W_LOG_TIME;
CREATE TABLE  IF NOT EXISTS  W_LOG_TIME (IPPORT VARCHAR(40), ID VARCHAR(40), URL VARCHAR(200), COUNT VARCHAR(10), TIME VARCHAR(30), COSTTIME VARCHAR(10) );
TRUNCATE TABLE W_LOG_TIME;
--接口访问记录
DROP TABLE W_LOG_INFO;
CREATE TABLE  IF NOT EXISTS  W_LOG_INFO (ID VARCHAR(40), TIME VARCHAR(30), USERID VARCHAR(40), URL VARCHAR(200), IP VARCHAR(32), MAC VARCHAR(64), PORT VARCHAR(10), ABOUT VARCHAR(512) );
TRUNCATE TABLE W_LOG_INFO;



---
--socket服务器 消息存储持久化
---
--消息 MERGE主表 分表ID 消息id - 消息json串
DROP TABLE W_MSG;
DROP TABLE W_MSG_0;
DROP TABLE W_MSG_1;
CREATE TABLE  IF NOT EXISTS  W_MSG_0 (ID VARCHAR(40) primary key, TEXT TEXT) ENGINE=MyISAM;
CREATE TABLE  IF NOT EXISTS  W_MSG_1 (ID VARCHAR(40) primary key, TEXT TEXT) ENGINE=MyISAM;

CREATE TABLE  IF NOT EXISTS  W_MSG (ID VARCHAR(40) primary key, TEXT TEXT) ENGINE=MERGE UNION=(W_MSG_0,W_MSG_1) INSERT_METHOD=FIRST;
-- TRUNCATE TABLE W_MSG_0;
-- TRUNCATE TABLE W_MSG_1;
-- TRUNCATE TABLE W_MSG;

--消息映射用户 MERGE主表 分表 ID 用户会话a:b - 消息id - 时间
DROP TABLE W_MSG_USER;
DROP TABLE W_MSG_USER_0;
DROP TABLE W_MSG_USER_1;
DROP TABLE W_MSG_USER_2;
DROP TABLE W_MSG_USER_3;
CREATE TABLE  IF NOT EXISTS  W_MSG_USER_0 (ID VARCHAR(200), USER_FROM VARCHAR(40), USER_TO VARCHAR(40), MSG_ID VARCHAR(40), TIME VARCHAR(20), primary key(ID,MSG_ID) ) ENGINE=MyISAM ;
CREATE TABLE  IF NOT EXISTS  W_MSG_USER_1 (ID VARCHAR(200), USER_FROM VARCHAR(40), USER_TO VARCHAR(40), MSG_ID VARCHAR(40), TIME VARCHAR(20), primary key(ID,MSG_ID) ) ENGINE=MyISAM ;
CREATE TABLE  IF NOT EXISTS  W_MSG_USER_2 (ID VARCHAR(200), USER_FROM VARCHAR(40), USER_TO VARCHAR(40), MSG_ID VARCHAR(40), TIME VARCHAR(20), primary key(ID,MSG_ID) ) ENGINE=MyISAM ;
CREATE TABLE  IF NOT EXISTS  W_MSG_USER_3 (ID VARCHAR(200), USER_FROM VARCHAR(40), USER_TO VARCHAR(40), MSG_ID VARCHAR(40), TIME VARCHAR(20), primary key(ID,MSG_ID) ) ENGINE=MyISAM ;

CREATE TABLE  IF NOT EXISTS  W_MSG_USER (ID VARCHAR(200), USER_FROM VARCHAR(40), USER_TO VARCHAR(40), MSG_ID VARCHAR(40), TIME VARCHAR(20), primary key(ID,MSG_ID) ) ENGINE=MERGE UNION=(W_MSG_USER_0,W_MSG_USER_1,W_MSG_USER_2,W_MSG_USER_3) INSERT_METHOD=FIRST;
-- TRUNCATE TABLE W_MSG_USER_0;
-- TRUNCATE TABLE W_MSG_USER_1;
-- TRUNCATE TABLE W_MSG_USER_2;
-- TRUNCATE TABLE W_MSG_USER_3;
-- TRUNCATE TABLE W_MSG_USER;





-- 分表分库 主数据源 分库源1 2
-- CREATE DATABASE IF NOT EXISTS walker0 default charset utf8 COLLATE utf8_general_ci;
-- GRANT ALL PRIVILEGES ON *.* TO 'walker0'@'%' IDENTIFIED BY 'qwer' WITH GRANT OPTION;
-- -- CREATE DATABASE IF NOT EXISTS walker0_master default charset utf8 COLLATE utf8_general_ci;
-- -- GRANT ALL PRIVILEGES ON *.* TO 'walker0_master'@'%' IDENTIFIED BY 'qwer' WITH GRANT OPTION;
-- -- CREATE DATABASE IF NOT EXISTS walker0_slave default charset utf8 COLLATE utf8_general_ci;
-- -- GRANT ALL PRIVILEGES ON *.* TO 'walker0_slave'@'%' IDENTIFIED BY 'qwer' WITH GRANT OPTION;
--
-- CREATE DATABASE IF NOT EXISTS walker1 default charset utf8 COLLATE utf8_general_ci;
-- GRANT ALL PRIVILEGES ON *.* TO 'walker1'@'%' IDENTIFIED BY 'qwer' WITH GRANT OPTION;



