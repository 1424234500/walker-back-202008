  ...
  methods: {
    ...
    clearRowSearch(){
     ...
     getListPage() {
      ...
      this.get('/redis/findPage.do', params).then((res) => {
...
//      newObj["S_FLAG"] = '1'
    ...
    handlerSave(){
      ...
      this.get('/redis/save.do', params).then((res) => {
        this.loadingSave = false
        this.loadingUpdate = ! this.loadingUpdate
      }).catch(() => {
        this.loadingSave = false
    ...
    handlerSaveAllRoles(){
      ...
      if(listOn.length > 0 || listOff.length > 0){
        var params = {ID: this.redisShowRole[this.colKey], ON: listOn.join(","), OFF: listOff.join(",")}
