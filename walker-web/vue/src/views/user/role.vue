<template>
  <div class="app-container" >
    ...
    >
      ...
      >
        ...
        >
          <template slot-scope="scope">
            ...
            <el-button size="mini" type="primary" icon="el-icon-tickets" circle @click.stop="handlerShowUser(scope.row)"></el-button>
            <el-button size="mini" type="primary" icon="el-icon-menu" circle @click.stop="handlerShowUser(scope.row)"></el-button>
      ...
      <el-dialog
        title="拥有该角色的用户/部门"
        :visible.sync="loadingColsRoleUser"
        width="86%"
      >
        <template>
          <!--搜索-->
          <div class="div-box-down"
               v-loading="! loadingColsRoleUser"
          >
            <form class="form-inline" >
              <div class="form-group" v-for="(value, key) in colMapRoleUser">
                <label>{{value=='' ? key : value}}</label>
                <input
                  type="text"
                  class="form-control"
                  style="width: 10em; margin-right: 1em;"
                  v-on:keyup.13="getListPageRoleUser()"
                  :placeholder="key"
                  v-model="rowSearch[key]"
                />
              </div>

              <el-button  class="btn btn-primary" @click="getListPageRoleUser()" >查询</el-button>
              <el-button  class="btn btn-default" @click="clearRowSearchRoleUser();getListPageRoleUser();" >清除</el-button>
            </form>
          </div>

          <div
            v-show="!loadingColsRoleUser"
          >
            <el-table
              v-loading="loadingListRoleUser"
              :data="list"
              :row-class-name="tableRowClassName"
              ref="multipleTable"
              @selection-change="handlerSelectionChangeRoleUser"
              highlight-current-row
              @sort-change="handlerSortChangeRoleUser"
              element-loading-text="Loading"
              border
              fit
              stripe
            ...
            >
              ...
              <!--      设置表头数据源，并循环渲染出来，property对应列内容的字段名，详情见下面的数据源格式 -->
              <el-table-column
                v-for="(value, key) in colMapRoleUser"
                :key="key"
                :property="key"
                :label="(value=='' ? key : value)"
                sortable
            ...
            <el-button
              v-if="rowSelectRoleUser.length > 0"
              class="btn btn-danger"
              @click="handlerDeleteAllRoleUser()"
              style="float:left;margin:4px 0px 0 2px;"
            >删除选中</el-button>
            <!--    分页-->
            <el-pagination
              @size-change="handlerSizeChangeRoleUser"
              :current-page="pageRoleUser.nowpage"
              :page-size="pageRoleUser.shownum"
              :page-sizes="[2, 4, 8, 16, 32, 64, 128]"
              :pager-count="9"
              layout="total, sizes, prev, pager, next, jumper"
              :total="pageRoleUser.num"
              background
              style="float:right;margin:10px 20px 0 0;"
            >
            </el-pagination>
  ...
  export default {
    ...
    data() {
      return {
        ...
        listRoleUser: [],
        colMapRoleUser: {},      //列名:别名
        colKeyRoleUser: "",     //主键名
        rowSearchRoleUser: {},   //搜索 列明:搜索值
        rowUpdateRoleUser: {},   //更新界面复制 列名:新值
        rowUpdateFromRoleUser: {},//更新界面源对象 列名:旧值
        rowSelectRoleUser: [],   //选中多行
        nowRowRoleUser: null,     //当前行
        pageRoleUser: {
          nowpage: 1,
          num: 0,
          order: "",
        ...
        loadingListRoleUser: true,
        loadingColsRoleUser: true,
        loadingSaveRoleUser: true,
        loadingUpdateRoleUser: false,
    ...
    methods: {
      ...
      handlerShowUser(val) {
        this.showRole = val
        this.loadingColsRoleUser = ! this.loadingColsRoleUser
        this.listRoleUser = []
        this.get('/common/getColsMap.do', {tableName: 'W_ROLE_USER'}).then((res) => {
          this.colMapRoleUser = res.data.colMap
          this.colKeyRoleUser = res.data.colKey
          this.getListPageRoleUser()
        }).catch(() => {
      ...
      //分页查询
      getListPageRoleUser() {
        this.loadingListRoleUser = true
        // name/nowPage/showNum
        var params = Object.assign({nowPage: this.pageRoleUser.nowpage, showNum: this.pageRoleUser.shownum, order: this.pageRoleUser.order}, this.rowSearchRoleUser)
        this.get('/role/findRoleUser.do', params).then((res) => {
          this.listRoleUser = res.data.data
          this.page = res.data.page
          this.loadingListRoleUser = false
        }).catch(() => {
          this.loadingListRoleUser = false
        })
      },
      ...
      handlerSaveAllRoles(){
        ...
        for(var i = 0; i < this.rowSelectRole.length; i++){
          map[this.rowSelectRole[i][this.colKeyUser]] = '1'
        }
        for(var i = 0; i < this.listRoleUser.length; i++){
          var roleUser = this.listRoleUser[i]
          var id = roleUser[this.colKeyUser]
        ...
        if(listOn.length > 0 || listOff.length > 0){
          var params = {ID: this.showRole[this.colKey], ON: listOn.join(","), OFF: listOff.join(",")}
